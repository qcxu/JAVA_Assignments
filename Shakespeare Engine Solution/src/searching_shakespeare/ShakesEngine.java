...
public class ShakesEngine {
    ...
    for (String line : lines) {
      ...
      while (scanner.hasNext()) {
        ...
        if (!index.containsKey(word)) {
        	...
        	numList.add(line_num);
        	index.put(word, numList);
        }
  ...
  public static void main(String[] args) throws IOException {
    ...
    if (index.containsKey(query)) {
      ...
      // it "retreived_lines"
    	ArrayList<Integer> line_numbers = index.get(query);
    	
      // **** TODO ****
      // Print out how many times the query word appeared in the text.
    	int times = line_numbers.size();
    	System.out.println(query + " appeared " + times + " times");
      ...
      // original lines stored in the arraylist "original_lines".
    	for (int i=0; i<times; i++) {
    		int line_num = line_numbers.get(i);
    		System.out.println(original_lines.get(line_num));
    	}
    } 
    else {
